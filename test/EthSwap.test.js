...
contract('EthSwap',([deployer , investor])=>{
    ...
    describe('buytokens()',async()=>{
        ...
        it('Allows user to instantly puchase tokens from ethswap for a fixed price',async ()=>{
           ...
           /// checking if value of ether has increased or not
           let ethSwapBalance
    ...
    describe('sellTokens()',async()=>{
        ...
        it('Allows user to instantly sell the tokens to ethSwap for a fixed price',async()=>{
            let inverstorBalance = await token.balanceOf(investor)
           ...
           //transaction history correct data
           const event = result.logs[0].args
           assert.equal(event.account,investor)
           assert.equal(event.token,token.address);
           assert.equal(event.amount.toString(),tokens('100').toString())
